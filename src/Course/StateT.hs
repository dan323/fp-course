...
--
-- >>> runStateT ((+1) <$> (StateT (\s -> (2,s):.Nil)) :: StateT Int List Int) 0
-- [(3,0)]
...
instance Monad k => Applicative (StateT s k) where
  ...
  (<*>) ::
    ...
    -> StateT s k a
    -> StateT s k b
  (<*>) (StateT f) (StateT g) = StateT (\s -> (\(h,s')-> (\(a,s'') -> pure (h a, s'')) =<< g s') =<< f s)
...
distinctG xs = runOptionalT $ evalT (filtering (\x -> StateT (OptionalT . log x) ) xs) S.empty
  where
    log x s 
      | x >100 = log1 ("aborting > 100: " ++ show' x) Empty
      | even x = log1 ("even number: " ++ show' x) (Full (S.notMember x s, S.insert x s))
      | otherwise = pure (Full (S.notMember x s, S.insert x s))
